...
void drawPose(cv::Mat& img, const cv::Mat& rot, float lineL)
{
	...
	//cout<<P.at<float>(0,3)<<" "<<P.at<float>(1,3)<<endl;
}
...
//#endif
	...
	while (key!=27) // Press Esc to quit
	{
		...
		if (isDetect)
		{
			...
			face_cascade.detectMultiScale(frame, faces, 1.2, 2, 0, cv::Size(60, 60));
		...
		else
		{
			...
			for (int i = 0 ; i < X0.cols ; i++)
				...
				//myfile<<(int)X0.at<float>(0,i)-319.5<<","<<(int)X0.at<float>(1,i)-239.5<<"\n";
				j++;
				if(j==14){
					//cout<<((int)X0.at<float>(0,i)-319.5)<<","<<((int)X0.at<float>(1,i)-239.5)<<"\n";
					myfile<<((int)X0.at<float>(0,i)-319.5)<<","<<((int)X0.at<float>(1,i)-239.5);
			}}
